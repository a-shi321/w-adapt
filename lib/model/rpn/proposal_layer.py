...
class _ProposalLayer(nn.Module):
    ...
    def forward(self, input):
        ...
        bbox_deltas = bbox_deltas.view(batch_size, -1, 4)  #[1, 33300, 4] #将RPN输出的边框变换信息维度变回[N,H,W,C]，再改变一下维度，变成[1×H×W×A,4]
        ...
        proposals = bbox_transform_inv(anchors, bbox_deltas, batch_size)  #bbox_transform_inv函数很简单，就是根据anchors和anchor的偏移量（tx, ty, tw, th）来生成 proposals
        ...
        _, order = torch.sort(scores_keep, 1, True)# 这里把是前景的分数进行排序，1代表以第2维进行排序，True代表从大到小# 返回的第一维是排好的tensor，第二维是index，这里只要index

        output = scores.new(batch_size, post_nms_topN, 5).zero_()  # 这里先定义了输出的tensor，一个(batch_size, post_nms_topN, 5)大小的全0矩阵
