...
if __name__ == '__main__':
  ...
  if args.dataset == "VOC2water":#partial-set
  ...
  if args.resume:
    load_name = os.path.join(output_dir,
      'voc2water_{}_{}_{}.pth'.format( args.checkepoch, args.checkpoint,args.step))
  ...
  for epoch in range(args.start_epoch, args.max_epochs + 1):
    ...
    with torch.no_grad():
        memory = Variable(torch.zeros((s_imdb.num_classes), 1).cuda())
    for step in range(1, iters_per_epoch + 1):
      ...
      rois_label, DA_img_loss_cls, DA_ins_loss_cls, tgt_DA_img_loss_cls, tgt_DA_ins_loss_cls,memory\
      = \
          fasterRCNN(im_data, im_info, gt_boxes, num_boxes,
                     tgt_im_data, tgt_im_info, tgt_gt_boxes, tgt_num_boxes,step,Variable(memory, volatile=True))
      ...
      if step % args.disp_interval == 0:
        ...
        if args.mGPUs:
          ...
          # img_loss_cls=img_loss_cls.item()
        ...
        print("[session %d][epoch %2d][iter %4d/%4d]partial-set:voc2water loss: %.4f, lr: %.2e" \
      ...
      if 4<epoch<16 and step%2000==0 and step!=10000:
        save_name = os.path.join(output_dir, 'voc2water_%d_%d_%d.pth' % (args.session, epoch, step))
        save_checkpoint({
    ...
    save_name = os.path.join(output_dir,
        'voc2water_%d_%d_%d.pth' % (args.session, epoch, step))
    save_checkpoint({
            'session': args.session,
            'epoch': epoch + 1,
            'model': fasterRCNN.module.state_dict() if args.mGPUs else fasterRCNN.state_dict(),
